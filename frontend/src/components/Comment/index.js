...
function Comment({comment}) {
    ...
    return (
        <div>
            { !update && !deleteComment && <div>
                <p>{comment.text}</p>
                { commentOwner && <div>
                    <i onClick={() => setUpdate(true)}class="fa-regular fa-pen-to-square"></i>
                    <i onClick={() => setDeleteComment(true)} class="fa-solid fa-trash-can"></i>
                </div>}
            ...
            { update && <div>
                <form>
                <textarea maxLength='256' minLength='5' type='text' value={updatedCommentText} placeholder='Update your comment...' onChange={((e) => setUpdatedCommentText(e.target.value))}></textarea>
