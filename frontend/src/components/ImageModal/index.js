...
function ImageModal({ imageAndArtist }) {
    ...
    const previewImageDetails = show.ShowImages.filter((showImage) => showImage.preview === true)[0]
    

    if (!image) {
        // console.log(previewImageDetails, '---previewImageDetails')
        return (
            <div id='image-modal'>
            <p id='leave-image-modal' onClick={closeModal}>X</p>
            <div className='image-modal-image-info-box'>
                <div id='image-modal-image-box'>
                    <img className='image-modal-image' src={previewImageDetails.imageUrl} alt={previewImageDetails.title}></img>
