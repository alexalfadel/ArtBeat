...
router.delete('/:rsvpId', async (req, res) => {
    ...
    if (!rsvp) return res.status(404).json({ message : "RSVP could not be found"})
    else if (rsvp.userId !== userId) return res.status(404).json({ message : "You must own the RSVP to delete it"})
